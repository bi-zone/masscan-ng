...
Masscan-ng is a fork of masscan. Main changes are SSL/TLS incapsulated connections
support (HTTPS in particular) and multithreaded packet processing. A bunch of
minor errors and leaks was fixed as well. Since the project contains a lot of
various functionality, most likely something was broken.

NOTE: masscan-ng uses its own **ad hoc TCP/IP stack**. Anything other than
...
use `--src-port` to configure which source ports masscan-ng uses, then also
...
or `clang`).
	...
	sudo apt-get --assume-yes install git make gcc libpcre3-dev libssl-dev
	git clone https://github.com/bi-zone/masscan-ng
	cd masscan-ng
	make

This puts the program in the `masscan-ng/bin` subdirectory. 
...
systems (Windows, macOS, etc.). Here's some additional build info:
  ...
  * Windows w/ Visual Studio: use the VS17 project
  * Windows w/ MinGW: just type `make`
  * Windows w/ cygwin: won't work
  * Mac OS X /w cmdline: just type `make`
  * FreeBSD: type `gmake`
  * other: try just compiling all the files together, `cc src/*.c -o bin/masscan-ng`
...
Masscan-ng can do more than just detect whether ports are open. It can also
complete the TCP connection and interaction with the application at that
port in order to grab simple "banner" information.

Masscan-ng supports banner checking on the following protocols:
  * FTP
  * HTTP/HTTPS
  * IMAP4
  * memcached
...
The problem with this is that masscan-ng contains its own TCP/IP stack
...
the connection before masscan-ng can grab the banner.

The easiest way to prevent this is to assign masscan-ng a separate IP
...
be used by another system. Masscan-ng will warn you that you've made a
...
firewall the port that masscan-ng uses. This prevents the local TCP/IP stack
from seeing the packet, but masscan-ng still sees it since it bypasses the
...
On FreeBSD and older MacOS, use an `ipfw` command: 
	...
	# masscan-ng 10.0.0.0/8 -p80 --banners --source-port 40000
...
are necessary. However, masscan-ng is still designed to work best using its
...
By default, masscan-ng first loads the configuration file 
`/etc/masscan-ng/masscan-ng.conf`. Any later configuration parameters override what's
...
By default, masscan-ng produces fairly large text files, but it's easy 
...
2. binary: This is the masscan-ng builtin format. It produces much smaller files so that
...
to `nmap` users, even though it's fundamentally different. Masscan-ng is tuned
...
You can think of `masscan-ng` as having the following settings permanently
enabled:
...
crash under the load that masscan-ng can generate.
...
A key difference between Masscan-ng and other scanners is the way it randomizes
targets.
...
Masscan-ng supports not only IP address ranges, but also port ranges. This means
...
Masscan-ng is designed for the next level of scalability, the "C10M problem".

The C10M solution is to bypass the kernel. There are three primary kernel
bypasses in Masscan-ng:
* custom network driver
* user-mode TCP stack
* user-mode synchronization

Masscan-ng can use the PF_RING DNA driver. This driver DMAs packets directly
...
Masscan-ng has its own built-in TCP stack for grabbing banners from TCP
...
Masscan-ng has no "mutex". Modern mutexes (aka. futexes) are mostly user-mode,
...
Instead, Masscan-ng uses "rings" to synchronize things, such as when the
...
Masscan-ng supports IPv6, but there is no special mode, both are supported
at the same time. (There is no `-6` option -- it's always available).

In any example you see of masscan-ng usage,
...
extra seconds to be read on startup (masscan-ng sorts the addresses and removes
duplicates before scanning).

Remember that masscan-ng contains its own network stack. Thus, the local machine
you run masscan-ng from does not need to be IPv6 enabled -- though the local
network needs to be able to route IPv6 packets.

### PF_RING

To get beyond 2 million packets/second, you need an Intel 10-gbps Ethernet
adapter and a special driver known as ["PF_RING ZC" from ntop](http://www.ntop.org/products/packet-capture/pf_ring/pf_ring-zc-zero-copy/).
Masscan-ng doesn't need to be rebuilt in order to use PF_RING. To use PF_RING,
...
When Masscan-ng detects that an adapter is named something like `zc:enp1s0` instead
...
to avoid overloading your local router:
    ...
    $ bin/masscan-ng 0.0.0.0/4 -p80 --rate 100000000 --router-mac 66-55-44-33-22-11
...
without the transmit overhead:
    ...
    $ bin/masscan-ng 0.0.0.0/4 -p80 --rate 100000000 --offline
...
at which they perform this calculation, making `masscan-ng` much faster.

## Licenses

This tool s distributed under GNU Affero GPLv3 license. See the LICENSE file for details.

## Authors/Acknowledgements

This tool was maintained and developed by Konstantin Molodyakov from BI.ZONE:  
email: k.molodyakov@bi.zone  
github: @sc07kvm  
