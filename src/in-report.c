...
/***************************************************************************
 ***************************************************************************/
static struct CNDB_Database *cndb_db = NULL;
...
static const char *cndb_lookup(unsigned ip) {
  ...
  entry = cndb_db->entries[ip & 0xFFFF];
...
static void cndb_add(unsigned ip, const unsigned char *name,
  ...
  if (cndb_db == NULL) {
    cndb_db = CALLOC(1, sizeof(*cndb_db));
  }
  ...
  entry->next = cndb_db->entries[ip & 0xFFFF];
  cndb_db->entries[ip & 0xFFFF] = entry;
}
...
static void xname_init(void) {
  ...
  for (i = 0; mynames[i].name; i++) {
    ...
    size_t len = mynames[i].length;
    size_t id = mynames[i].code;
...
static unsigned found_type(const unsigned char *banner, size_t banner_length) {
  ...
  id = smack_search_next(global_xnames, &state, banner, &offset, banner_length);
...
void readscan_report(unsigned ip, unsigned app_proto, unsigned char **r_data,
                     size_t *r_data_length, size_t r_data_max_length) {

  size_t data_length = *r_data_length;
  unsigned char *data = *r_data;
  UNUSEDPARM(r_data_max_length);
