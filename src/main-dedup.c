...
/* This is the number of entries in our table. More entries does a better job
...
static inline unsigned dedup_hash_ipv6(const ipaddress *ip_them,
                                       unsigned port_them,
...
static inline int is_equal6(const ipv6address_t *lhs,
...
static unsigned dedup_is_duplicate_ipv6(struct DedupTable *dedup,
  ...
  memmove(bucket, bucket + 1,
          (ARRAY_SIZE(dedup->entries6[0]) - 1) * sizeof(*bucket));
...
static unsigned dedup_is_duplicate_ipv4(struct DedupTable *dedup,
   ...
   * older entries at this bucket off the list */
  memmove(bucket, bucket + 1,
          (ARRAY_SIZE(dedup->entries[0]) - 1) * sizeof(*bucket));
...
int dedup_selftest(void) {
   ...
   */
  {
    ...
    if (!dedup_is_duplicate(dedup, &ip_them, port_them, &ip_me, port_me)) {
      ...
      ipaddress_fmt(&fmt1, &ip_them);
      ipaddress_fmt(&fmt2, &ip_me);
