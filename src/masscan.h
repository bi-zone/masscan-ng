...
enum Operation {
  ...
  Operation_List_Adapters = 1, /* --iflist */
  ...
  Operation_NmapHelp = 11,     /* --nmap */
  Operation_Version = 12,      /* --version */
};
...
struct Masscan {
  ...
  unsigned is_ssl_dynamic : 1; /* --ssl-dynamic, use OpenSSL for ssl connect */
  unsigned
  ...
  unsigned is_regex_only_banners : 1; /* --regex-only-banners */
  unsigned is_dynamic_set_host : 1;   /* --dynamic-set-host */
  unsigned is_echo_all : 1;           /* --echo-all */
  unsigned is_tranquility : 1;        /* --is_tranquility */
  ...
  /* --num-handle-threads */
  size_t recv_handle_thread_count;
  ...
  struct OutputStuff {
     ...
     * The name of the file where we are dumping ssl keys.
     * Note: the filename "-" means that we should send the file to
     * <stdout> rather than to a file. */
    char filename_ssl_keys[256];
