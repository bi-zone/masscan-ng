...
struct ipaddress_formatted *ipaddress_fmt(struct ipaddress_formatted *out,
                                          const ipaddress *a) {
  stream_t s;
  ipv4address_t ip = a->ipv4;

  if (a->version == 6) {
    return ipv6address_fmt(out, &a->ipv6);
  }
...
unsigned massint128_bitcount(const massint128_t *number) {
  if (number->hi)
...
int ipv6address_is_equal_prefixed(const ipv6address_t *p_lhs,
                                  const ipv6address_t *p_rhs, unsigned prefix) {
  ipv6address_t mask;
  ipv6address_t lhs = *p_lhs;
  ipv6address_t rhs = *p_rhs;
