...
typedef unsigned ipv4address_t;
...
typedef struct macaddress macaddress_t;
...
typedef ipv6address_t massint128_t;
...
static inline int ipv6address_is_zero(const ipv6address_t *a) {
  return a->hi == 0 && a->lo == 0;
}
#define massint128_is_zero ipv6address_is_zero

static inline int ipv6address_is_invalid(const ipv6address_t *a) {
  return a->hi == ~0ULL && a->lo == ~0ULL;
}

static inline int ipv6address_is_equal(const ipv6address_t *a,
                                       const ipv6address_t *b) {
  return a->hi == b->hi && a->lo == b->lo;
}

static inline int ipv6address_is_lessthan(const ipv6address_t *a,
                                          const ipv6address_t *b) {
  return (a->hi == b->hi) ? (a->lo < b->lo) : (a->hi < b->hi);
}

int ipv6address_is_equal_prefixed(const ipv6address_t *lhs,
                                  const ipv6address_t *rhs, unsigned prefix);

static inline ipv6address_t *ipv6address_from_bytes(ipv6address_t *addr,
...
static inline macaddress_t *macaddress_from_bytes(macaddress_t *result,
                                                  const void *vbuf) {
...
static inline int macaddress_is_zero(const macaddress_t *mac) {
  return mac->addr[0] == 0 && mac->addr[1] == 0 && mac->addr[2] == 0 &&
         mac->addr[3] == 0 && mac->addr[4] == 0 && mac->addr[5] == 0;
}

static inline int macaddress_is_equal(const macaddress_t *lhs,
                                      const macaddress_t *rhs) {
...
struct ipaddress_formatted *ipv6address_fmt(struct ipaddress_formatted *out,
                                            const ipv6address_t *a);
struct ipaddress_formatted *ipv4address_fmt(struct ipaddress_formatted *out,
                                            const ipv4address_t *a);
struct ipaddress_formatted *ipaddress_fmt(struct ipaddress_formatted *out,
                                          const ipaddress *a);
struct ipaddress_formatted *macaddress_fmt(struct ipaddress_formatted *out,
                                           const macaddress_t *a);
unsigned massint128_bitcount(const massint128_t *num);
