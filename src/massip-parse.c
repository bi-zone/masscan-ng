...
enum parser_result_t { Still_Working, Found_Error, Found_IPv4, Found_IPv6 };

struct massip_parser {
  uint64_t line_number;
  uint64_t char_number;
...
/***************************************************************************
 ***************************************************************************/
static void _parser_err(struct massip_parser *p, uint64_t *line_number,
                        uint64_t *charindex) {
...
static unsigned _parser_finish_ipv6(struct massip_parser *p) {
  size_t index = p->ipv6.index;
  size_t ellision = p->ipv6.ellision_index;
 ...
 ***************************************************************************/
static void _parser_get_ipv6(struct massip_parser *state, ipv6address_t *begin,
                             ipv6address_t *end) {
 ...
 ***************************************************************************/
static enum parser_state_t _switch_to_ipv6(struct massip_parser *p,
                                           enum parser_state_t old_state) {
  unsigned num = p->tmp;
 ...
 */
static void _ipv6_apply_cidr(ipv6address_t *begin, ipv6address_t *end,
                             unsigned prefix) {
  ipv6address_t mask;

  /* For bad prefixes, make sure we return an invalid address */
  if (prefix > 128) {
    static const ipv6address_t invalid = {~0ULL, ~0ULL};
    *begin = invalid;
    *end = invalid;
    return;
  };
 ...
 * 'state-machine parser'.
 ***************************************************************************/
static enum parser_result_t _parser_next(struct massip_parser *p,
                                         const char *buf, size_t *r_offset,
                                         size_t length, ipv4address_t *r_begin,
                                         ipv4address_t *r_end) {

  size_t i;
  enum parser_state_t state = p->state;
  enum parser_result_t result = Still_Working;
 ...
 ***************************************************************************/
static int rangefile_test_error(const char *buf, uint64_t in_line_number,
                                uint64_t in_char_number, unsigned which_test) {
  ...
  uint64_t out_char_number;
  enum parser_result_t x;
...
fail:
  _parser_destroy(p);
  LOG(LEVEL_WARNING, "[-] rangefile test fail, line=%u\n", which_test);
  return 1;
}
...
int massip_parse_file(struct MassIP *massip, const char *filename) {
  ...
  FILE *fp = NULL;
  int err;
  enum parser_result_t err_parse_next;

  bool is_error = false;
  size_t addr_count = 0;
  uint64_t line_number, char_number;
  ...
  while (!is_error) {
    ...
    while (offset < count) {
      ...
      err_parse_next = _parser_next(p, buf, &offset, count, &begin, &end);
      switch (err_parse_next) {
      case Still_Working:
        if (offset < count) {
          /* We reached this somehow in the middle of the buffer, but
           * this return is only possible at the end of the buffer */
          LOG(LEVEL_WARNING, "[-] rangeparse_next(): unknown coding failure\n");
        }
        break;
      case Found_Error:
      default:
        ...
        LOG(LEVEL_WARNING,
            "[-] %s:%llu:%llu: invalid IP address on line #%llu\n", filename,
      ...
      case Found_IPv4:
        ...
        break;
      case Found_IPv6: {
        ipv6address_t found_begin, found_end;
  ...
  if (!is_error) {
    ...
    default:
      _parser_err(p, &line_number, &char_number);
      LOG(LEVEL_WARNING, "[-] %s:%llu:%llu: invalid IP address on line #%llu\n",
  ...
  LOG(LEVEL_INFO, "[+] %s: %" PRIuPTR " addresses read\n", filename,
      addr_count);
...
ipv6address_t *massip_parse_ipv6(ipv6address_t *result_ip, const char *line) {
  struct massip_parser p[1];
  size_t count = strlen(line);
  size_t offset = 0;
  enum parser_result_t err;
  unsigned begin, end;
  ipv6address_t range;
...
again:
  ...
  case Still_Working:
    if (offset < count) {
      /* We reached this somehow in the middle of the buffer, but
       * this return is only possible at the end of the buffer */
      LOG(LEVEL_WARNING, "[-] _parser_next(): unknown coding failure\n");
      goto fail;
    } else {
      err = _parser_next(p, "\n", 0, 1, &begin, &end);
  ...
  default:
    goto fail;
  case Found_IPv6:
    _parser_get_ipv6(p, result_ip, &range);
    if (!ipv6address_is_equal(result_ip, &range)) {
      goto fail;
    }
    return result_ip;
  }
...
ipv4address_t *massip_parse_ipv4(ipv4address_t *result_ip, const char *line) {
  struct massip_parser p[1];
  size_t count = strlen(line);
  size_t offset = 0;
  enum parser_result_t err;
  ipv4address_t begin, end;
...
static int selftest_massip_parse_range(void) {
  ...
  for (i = 0; cases[i].line; i++) {
    ...
    while (offset < length) {
      enum RangeParseResult x;
      x = massip_parse_range(cases[i].line, &offset, length, &range4, &range6);
      switch (x) {
      default:
      case Bad_Address:
        LOG(LEVEL_ERROR, "[-] selftest_massip_parse_range[%" PRIu64 "] fail\n",
            i);
        return 1;
      case Ipv4_Address:
        if (cases[i].list[j].ipv4.begin != range4.begin ||
          ...
          LOG(LEVEL_ERROR,
              "[-] selftest_massip_parse_range[%" PRIu64 "] fail\n", i);
          return 1;
        }
        break;
      }
...
static int rangefile6_test_buffer(struct massip_parser *parser, const char *buf,
                                  const ipv6address_t *expected_begin,
                                  const ipv6address_t *expected_end) {

  size_t length = strlen(buf);
  size_t offset = 0;
  ipv6address_t found_begin = {1, 2};
  ipv6address_t found_end = {1, 2};
  unsigned tmp1, tmp2;
  enum parser_result_t err;
  ...
  case Found_IPv6:
    ...
    if (!ipv6address_is_equal(&found_begin, expected_begin)) {
      ...
      ipv6address_fmt(&fmt1, &found_begin);
      ipv6address_fmt(&fmt2, expected_begin);
      LOG(LEVEL_WARNING, "[-] begin mismatch: found=[%s], expected=[%s]\n",
    ...
    if (!ipv6address_is_equal(&found_end, expected_end)) {
      ipaddress_formatted_t fmt1;
      ipaddress_formatted_t fmt2;
      ipv6address_fmt(&fmt1, &found_end);
      ipv6address_fmt(&fmt2, expected_end);
      LOG(LEVEL_WARNING, "[-] end mismatch: found=[%s], expected=[%s]\n",
          fmt1.string, fmt2.string);
      goto fail;
    }
    break;
...
int massip_parse_selftest(void) {
  ...
  for (i = 0; test_cases[i].string; i++) {
    ...
    if (x) {
      LOG(LEVEL_ERROR, "[-] failed: %" PRIu64 ": %s\n", i,
          test_cases[i].string);
      break;
    }
  }
