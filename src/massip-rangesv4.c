...
#define BAD_BEGIN_RANGE 0xFFFFFFFF
#define BAD_END_RANGE 0
#define SET_BAD_RANGE(x)                                                       \
  {                                                                            \
    (x)->begin = BAD_BEGIN_RANGE;                                              \
    (x)->end = BAD_END_RANGE;                                                  \
  }
 ...
 * as a linear search.
 ***************************************************************************/
int rangelist_is_contains(const struct RangeList *targets,
                          const ipv4address_t *addr) {
  size_t i;
...
static void rangelist_remove_at(struct RangeList *targets, size_t index) {
  memmove(&targets->list[index], &targets->list[index + 1],
          (targets->count - (index + 1)) * sizeof(targets->list[0]));
  targets->count--;
}
...
void rangelist_sort(struct RangeList *targets) {
  ...
  /* First, sort the list */
  LOG(LEVEL_DEBUG_1, "[+] range:sort: sorting...\n");
  ...
  LOG(LEVEL_DEBUG, "[+] range:sort: done...\n");
 ...
 * its replacement.
 ***************************************************************************/
void rangelist_remove_range(struct RangeList *targets, unsigned begin,
                            unsigned end) {
  size_t i;
  struct Range x;
...
void rangelist_add_range2(struct RangeList *targets, struct Range *range) {
  if (range == NULL) {
    return;
  }
...
void rangelist_remove_range2(struct RangeList *targets, struct Range *range) {
  if (range == NULL) {
    return;
  }
 ...
 ***************************************************************************/
static int parse_ipv4(const char *line, size_t *inout_offset, size_t max,
                      ipv4address_t *ipv4) {
 ...
 ****************************************************************************/
struct Range *range_parse_ipv4(const char *line, size_t *inout_offset,
                               size_t max, struct Range *result) {

  size_t offset;
  int err;

  if (line == NULL) {
    SET_BAD_RANGE(result);
    return NULL;
  }
  ...
  if (line[offset] == '/') {
    ...
    while (offset < max && isdigit(line[offset] & 0xFF)) {
      prefix = prefix * 10 + ((uint64_t)line[offset++] - '0');
      if (++digits > 2) {
        SET_BAD_RANGE(result);
        return NULL;
      }
...
/***************************************************************************
 ***************************************************************************/
int range_is_valid(const struct Range range) {
...
void rangelist_optimize(struct RangeList *targets) {
  ...
  for (i = 0; i < targets->count; i++) {
    picker[i] = total;
    total += (size_t)targets->list[i].end - (size_t)targets->list[i].begin + 1;
  }
  ...
  *is_error = 0;
  while (*p) {
    ...
    if (isalpha(*p & 0xFF) && p[1] == ':') {
      ...
      case 'O':
      case 'o':
        proto_offset = Templ_Oproto;
        break;
      case 'I':
      case 'i':
        proto_offset = Templ_ICMP;
        break;
      default:
...
static int exclude_selftest(void) {
  ...
  rangelist_remove_all(&includes1);
  rangelist_remove_all(&includes2);
  rangelist_remove_all(&excludes);
...
int ranges_selftest(void) {
   ...
   * entire Internet */
  range_parse_ipv4("0.0.0.0/0", NULL, 0, &r);
  REGRESS(r.begin == 0 && r.end == 0xFFFFFFFF);

  p_r = range_parse_ipv4("0.0.0./0", NULL, 0, &r);
  REGRESS(p_r == NULL);

  p_r = range_parse_ipv4("75.748.86.91", NULL, 0, &r);
  REGRESS(p_r == NULL);

  p_r = range_parse_ipv4("23.75.345.200", NULL, 0, &r);
  REGRESS(p_r == NULL);

  p_r = range_parse_ipv4("192.1083.0.1", NULL, 0, &r);
  REGRESS(p_r == NULL);

  range_parse_ipv4("192.168.1.3", NULL, 0, &r);
  if (r.begin != 0xc0a80103 || r.end != 0xc0a80103) {
    LOG(LEVEL_ERROR, "r.begin = 0x%08x r.end = 0x%08x\n", r.begin, r.end);
    REGRESS_ERROR();
  }

  range_parse_ipv4("10.0.0.20-10.0.0.30", NULL, 0, &r);
  if (r.begin != 0x0A000000 + 20 || r.end != 0x0A000000 + 30) {
    LOG(LEVEL_ERROR, "r.begin = 0x%08x r.end = 0x%08x\n", r.begin, r.end);
    REGRESS_ERROR();
  }

  range_parse_ipv4("10.0.1.2/16", NULL, 0, &r);
  if (r.begin != 0x0A000000 || r.end != 0x0A00FFFF) {
    LOG(LEVEL_ERROR, "r.begin = 0x%08x r.end = 0x%08x\n", r.begin, r.end);
    REGRESS_ERROR();
  }

  rangelist_add_range2(targets, range_parse_ipv4("10.0.0.0/24", NULL, 0, &r));
  rangelist_add_range2(targets,
                       range_parse_ipv4("10.0.1.10-10.0.1.19", NULL, 0, &r));
  rangelist_add_range2(targets,
                       range_parse_ipv4("10.0.1.20-10.0.1.30", NULL, 0, &r));
  rangelist_add_range2(targets,
                       range_parse_ipv4("10.0.0.0-10.0.1.12", NULL, 0, &r));
  rangelist_sort(targets);

  if (targets->count != 1) {
    LOG(LEVEL_ERROR, "count = %" PRIuPTR "\n", targets->count);
    REGRESS_ERROR();
  }
  ...
  rangelist_add_range2(targets, range_parse_ipv4("10.0.0.0/8", NULL, 0, &r));
  rangelist_sort(targets);

  /* These removals shouldn't change anything */
  rangelist_remove_range2(targets,
                          range_parse_ipv4("9.255.255.255", NULL, 0, &r));
  rangelist_remove_range2(targets,
                          range_parse_ipv4("11.0.0.0/16", NULL, 0, &r));
  rangelist_remove_range2(targets,
                          range_parse_ipv4("192.168.0.0/16", NULL, 0, &r));
  rangelist_sort(targets);
  ...
  rangelist_remove_range2(targets,
                          range_parse_ipv4("1.0.0.0-10.0.0.0", NULL, 0, &r));
  rangelist_remove_range2(
      targets, range_parse_ipv4("10.255.255.255-11.0.0.0", NULL, 0, &r));
  rangelist_sort(targets);
  ...
  rangelist_remove_range2(targets,
                          range_parse_ipv4("10.10.0.0/16", NULL, 0, &r));
  rangelist_remove_range2(targets,
                          range_parse_ipv4("10.20.0.0/16", NULL, 0, &r));
  rangelist_sort(targets);
  if (targets->count != 3) {
    REGRESS_ERROR();
  }
  ...
  rangelist_remove_range2(targets,
                          range_parse_ipv4("10.12.0.0/16", NULL, 0, &r));
  rangelist_sort(targets);
  if (targets->count != 4) {
    REGRESS_ERROR();
  }
  ...
  rangelist_remove_range2(
      targets, range_parse_ipv4("10.10.10.10-10.12.12.12", NULL, 0, &r));
  rangelist_sort(targets);
  if (targets->count != 3) {
    REGRESS_ERROR();
  }
