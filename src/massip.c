...
void massip_apply_excludes(struct MassIP *targets, struct MassIP *exclude) {
  ...
  range6list_exclude(NULL, &targets->ipv6, &exclude->ipv6);
...
void massip_optimize(struct MassIP *targets) {
  ...
  targets->count_ipv6s = range6list_count(&tmp, &targets->ipv6)->lo;
...
int massip_pick(struct MassIP *massip, uint64_t index, ipaddress *addr,
  ...
  if (index < massip->ipv4_index_threshold) {
    ...
    *port = rangelist_pick(&massip->ports, index / massip->count_ipv4s);
    ...
    range6list_pick(&addr->ipv6, &massip->ipv6, index % massip->count_ipv6s);
...
void massip_free(struct MassIP *massip) {
...
int massip_selftest(void) {
  ...
  line = __LINE__;
  massip_range(&count, &targets);
  if (count.hi != 0 || count.lo != 12) {
    goto fail;
  }

  massip_free(&targets);
  massip_free(&excludes);
  return 0;
fail:
  LOG(LEVEL_WARNING, "[-] massip: test fail, line=%d\n", line);
  return 1;
}
