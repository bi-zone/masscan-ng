...
const char *reason_string(int x, char *buffer, size_t sizeof_buffer) {
  sprintf_s(buffer, sizeof_buffer, "%s%s%s%s%s%s%s%s", (x & 0x01) ? "fin-" : "",
...
static FILE *open_rotate(struct Output *out, const char *filename) {
  ...
  if (out->format == Output_Redis) {
    ...
    if (fd < 1) {
      ...
      if (fd == -1) {
        LOG(LEVEL_ERROR, "redis: socket() failed to create socket\n");
        exit(1);
      }
      ...
      if (x != 0) {
        LOG(LEVEL_WARNING, "redis: connect() failed\n");
        LOG(LEVEL_WARNING, "connect: %s\n", strerror(errno));
      }
      out->redis.fd = fd;
    }
    out->funcs->open(out);
  ...
  if (fp == NULL) {
    ...
    if (x != 0 || fp == NULL) {
      LOG(LEVEL_ERROR, "out: could not open file for %s\n",
...
static FILE *open_ssl_keys(struct Output *out, const char *filename) {
  FILE *fp = NULL;
  unsigned is_append = out->is_append;
  int x;
...
static void close_ssl_keys(struct Output *out, FILE *fp) {
  if (out == NULL || fp == NULL) {
    return;
  }
 ...
 *****************************************************************************/
static time_t next_rotate_time(time_t last_rotate, time_t period,
                               time_t offset) {

  time_t next = last_rotate - (last_rotate % period) + period + offset;
  return next;
}
...
/*****************************************************************************
 *****************************************************************************/
static int ends_with(const char *filename, const char *extension) {
 ...
 *****************************************************************************/
static char *indexed_filename(const char *filename, size_t index,
                              size_t sub_index) {
  size_t len = strlen(filename);
  size_t ext;
  char *new_filename;
  size_t new_length = strlen(filename) + 64;
  ...
  /* format the new name */
  sprintf_s(new_filename, new_length, "%.*s.%02" PRIuPTR ".%02" PRIuPTR "%s",
            (unsigned)ext, filename, index, sub_index, filename + ext);
  return new_filename;
}
...
struct Output *output_create(const struct Masscan *masscan, size_t index,
  ...
  out->xml.stylesheet = STRDUP(masscan->output.stylesheet);
  out->rotate.directory = STRDUP(masscan->output.rotate.directory);

  if (out->rotate.period != 0 || out->rotate.offset != 0) {
    if (masscan->output.filename[0] == '-' &&
        masscan->output.filename[1] == '\0') {
      LOG(LEVEL_ERROR, "Rotate on stdout not support\n");
      exit(1);
    }
  }

  if ((masscan->nic_count <= 1 && masscan->recv_handle_thread_count <= 1) ||
      (masscan->output.filename[0] == '-' &&
       masscan->output.filename[1] == '\0')) {
    out->filename = STRDUP(masscan->output.filename);
  } else {
    out->filename =
        indexed_filename(masscan->output.filename, index, sub_index);
  }

  if ((masscan->nic_count <= 1 && masscan->recv_handle_thread_count <= 1) ||
      (masscan->output.filename_ssl_keys[0] == '-' &&
       masscan->output.filename_ssl_keys[1] == '\0')) {
    out->filename_ssl_keys = STRDUP(masscan->output.filename_ssl_keys);
  } else {
    out->filename_ssl_keys =
        indexed_filename(masscan->output.filename_ssl_keys, index, sub_index);
  }
  ...
  if (out->filename[0] && out->funcs != &null_output) {
    FILE *fp;
    fp = open_rotate(out, out->filename);
    if (fp == NULL) {
  ...
  if (out->filename_ssl_keys[0]) {
    FILE *fp;
    fp = open_ssl_keys(out, out->filename_ssl_keys);
    if (fp == NULL) {
      LOG(LEVEL_ERROR, "%s: %s\n", out->filename_ssl_keys, strerror(errno));
      exit(1);
    }
    out->fp_ssl_keys = fp;
  }
...
static FILE *output_do_rotate(struct Output *out, int is_closing) {
   ...
   * to start with */
  slash_filename = strchr(filename, '/');
  while (slash_filename) {
    filename = slash_filename;
    if (*filename == '/')
      filename++;
    slash_filename = strchr(filename, '/');
  }
  ...
  if (err != 0) {
    free(new_filename);
    LOG(LEVEL_WARNING, "gmtime(): file rotation ended: %s\n", strerror(errno));
    return out->fp;
  }
...
again:
  if (out->rotate.filesize) {
    ...
    char *dot_in_filename = strrchr(filename, '.');
    if (dot_in_filename) {
      x_off = dot_in_filename - filename;
      x_len = strlen(filename + x_off);
    } else {
      x_off = strlen(filename);
      x_len = 0;
    }
    sprintf_s(new_filename, new_filename_size, "%s/%.*s-%05u%.*s", dir,
              (int)x_off, filename, out->rotate.filecount++, (int)x_len,
              filename + x_off);
  } else {
    sprintf_s(new_filename, new_filename_size,
              "%s/%02u%02u%02u-%02u%02u%02u"
              "-%s",
              dir, (unsigned)(tm.tm_year % 100), (unsigned)(tm.tm_mon + 1),
              (unsigned)tm.tm_mday, (unsigned)tm.tm_hour, (unsigned)tm.tm_min,
              (unsigned)tm.tm_sec, filename);
  ...
  if (err) {
    LOG(LEVEL_WARNING, "rename(\"%s\", \"%s\"): failed\n", filename,
        new_filename);
    LOG(LEVEL_WARNING, "rename(): %s\n", strerror(errno));
    free(new_filename);
    return out->fp;
  }
  ...
  if (is_closing)
    ...
    if (fp == NULL) {
      LOG(LEVEL_WARNING, "rotate: %s: failed: %s\n", filename, strerror(errno));
    } else {
      close_rotate(out, out->fp);
      out->fp = fp;
      out->rotate.last = time(0);
      LOG(LEVEL_INFO, "rotate: started new file: %s\n", filename);
    }
  }
  return out->fp;
}
...
/***************************************************************************
 ***************************************************************************/
static int is_rotate_time(const struct Output *out, time_t now) {
  if (out->is_virgin_file) {
    return 0;
  }
  ...
  if (out->rotate.filesize != 0 &&
      ftell_x(out->fp) >= (int64_t)out->rotate.filesize) {
    return 1;
  }
  return 0;
}
...
void output_report_sign(struct Output *out, time_t now, const ipaddress *ip,
   ...
   * line screen */
  if (out->is_interactive || out->format == Output_Default ||
      out->format == Output_Interactive) {
    unsigned count;

    count = fprintf(stdout, "Sign on port %u/%s on %s: %s", port,
                    name_from_ip_proto(ip_proto), fmt.string,
                    masscan_app_to_string(app_proto));
  ...
  if (is_rotate_time(out, now)) {
    FILE *fp = output_do_rotate(out, 0);
    if (fp == NULL) {
      LOG(LEVEL_ERROR, "%s: %s\n", out->filename, strerror(errno));
      exit(1);
    }
   ...
   * and so on. */
  out->funcs->sign(out, now, ip, ip_proto, port, app_proto);
}
 ...
 * back.
 ***************************************************************************/
void output_report_status(struct Output *out, time_t timestamp,
  ...
  default:
    assert(false);
    LOG(LEVEL_WARNING, "unknown status type: %u\n", status);
    return;
  }
  ...
  /* Now do the actual output, whether it be XML, binary, JSON, ndjson, Redis,
   * and so on. */
  out->funcs->status(out, timestamp, status, ip, ip_proto, port, reason, ttl);
}

/***************************************************************************
 ***************************************************************************/
void output_report_banner(struct Output *out, time_t now, const ipaddress *ip,
                          unsigned ip_proto, unsigned port,
  ...
  /* Now do the actual output, whether it be XML, binary, JSON, ndjson, Redis,
   * and so on.*/
  out->funcs->banner(out, now, ip, ip_proto, port, app_proto, ttl, px, length);
}

void output_ssl_key(struct Output *out, const char *line) {
  /* If not outputting to a file, then don't do anything */
  if (out->fp_ssl_keys == NULL) {
    return;
  }

  fprintf(out->fp_ssl_keys, "%s\n", line);
}
...
void output_destroy(struct Output *out) {
  ...
  if (out->rotate.period || out->rotate.filesize) {
    LOG(LEVEL_INFO, "doing finale rotate\n");
  ...
  if (out->fp_ssl_keys) {
    close_ssl_keys(out, out->fp_ssl_keys);
  }

  free(out->xml.stylesheet);
  free(out->rotate.directory);
  free(out->filename);
  free(out->filename_ssl_keys);
  free(out);
}
...
int output_selftest(void) {
  ...
  f = indexed_filename("foo.bar", 1, 0);
  if (strcmp(f, "foo.01.00.bar") != 0) {
    LOG(LEVEL_ERROR, "output: failed selftest\n");
    return 1;
  }
  free(f);

  f = indexed_filename("foo.b/ar", 2, 1);
  if (strcmp(f, "foo.b/ar.02.01") != 0) {
    LOG(LEVEL_ERROR, "output: failed selftest\n");
    return 1;
  }
  free(f);

  f = indexed_filename(".foobar", 3, 5);
  if (strcmp(f, ".03.05.foobar") != 0) {
    LOG(LEVEL_ERROR, "output: failed selftest\n");
    return 1;
  }
  free(f);
