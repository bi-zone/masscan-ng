...
struct Banner1 {
  ...
  unsigned is_regex_only_banners : 1;
  unsigned is_dynamic_set_host : 1;

  pcre *regex;
  pcre_extra *regex_extra;

  unsigned is_ssl_dynamic : 1;
  SSL_CTX *ssl_ctx;

  struct {
    struct ProtocolParserStream *tcp[COUNT_TCP_PORTS];
  } payloads;
...
struct SSLDYNAMICECORD {
  ...
  struct ProtocolParserStream *psub_stream;
...
#define ERROR_STATE (~((size_t)0))

struct ProtocolState {
  size_t state;
  size_t remaining;
  size_t detect_offset;

  ipaddress ip;
  unsigned short port;
  enum ApplicationProtocol app_proto;
  const struct ProtocolParserStream *parser_stream;

  unsigned is_sent_sslhello : 1;
  unsigned is_check_regexp : 1;

  struct BannerBase64 base64;

  union {
    struct SSLRECORD ssl;
    struct SSLDYNAMICECORD ssl_dynamic;
    struct VNCSTUFF vnc;
    struct FTPSTUFF ftp;
...
#define NOT_HAVE_OFFSET (~((size_t)0))
struct Patterns {
