...
/****************************************************************************
 ****************************************************************************/
static struct CoapLink *parse_links(const unsigned char *px, size_t offset,
                                    size_t length, size_t *r_count) {
  ...
  for (; offset < length; offset++)
    ...
    case PARM_QUOTED_ESCAPE:
      ...
      break;
    default:
      LOG(LEVEL_WARNING, "invalid state\n");
      state = INVALID;
      break;
    }
...
static bool coap_parse(const unsigned char *px, size_t length,
  ...
  if (length < 4) {
    LOG(LEVEL_DEBUG_1, "[-] CoAP: short length\n");
  ...
  *request_id = px[2] << 8 | px[3];
  ...
  if (version != 1) {
    LOG(LEVEL_DEBUG_1, "[-] CoAP: version=%u\n", version);
    goto not_this_protocol;
  }

  /* Only ACKs suported */
  if (type != 2) {
    LOG(LEVEL_DEBUG_1, "[-] CoAP: type=%u\n", type);
    goto not_this_protocol;
  }
  ...
  if (token_length > 8 || 4 + token_length > length) {
    LOG(LEVEL_DEBUG_1, "[-] CoAP: token-length=%" PRIuPTR "\n", token_length);
    goto not_this_protocol;
  }
  ...
  {
    char buf[64];
    sprintf_s(buf, sizeof(buf), "rsp=%u.%u(%s)", code >> 5, code & 0x1F,
              response_code(code));
  ...
  case 40:
    banout_append(banout, PROTO_COAP, " application/link-format", AUTO_LEN);
    {
      ...
      links = parse_links(px, offset, length, &count);
      for (i = 0; i < count; i++) {
...
/***************************************************************************
 ***************************************************************************/
unsigned coap_handle_response(struct Banner1 *banner1, struct Output *out,
...
static int test_is_link(const char *name, const unsigned char *vinput,
  ...
  LOG(LEVEL_ERROR, "[-] proto-coap failed at line number %d\n", line_number);
  return 0; /* not found */
}
...
int proto_coap_selftest(void) {
  ...
  /* test quoted */
  {
    static const unsigned char *input =
        (const unsigned char
             *)"</sensors/temp>;if=\"se\\\"\\;\\,\\<\\>\\\\nsor\",</success>";
    links = parse_links(input, 0, ~((size_t)0), &count);
