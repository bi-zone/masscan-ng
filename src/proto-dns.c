  ...
  for (;;) {
    ...
    case 0:
      ...
      if (px[offset] == 0) {
        ...
        name_length += (size_t)px[offset] + 1;
...
/****************************************************************************
 ****************************************************************************/
static void dns_extract_name(const unsigned char px[], size_t offset,
                             size_t max, struct DomainPointer *name) {
...
void proto_dns_parse(struct DNS_Incoming *dns, const unsigned char px[],
  ...
  static const size_t MAX_RRs = ARRAY_SIZE(dns->rr_offset);
  size_t i;
  ...
  */
  ...
  offset += 12;
  dns->is_valid = true;
  dns->is_formerr = true; /* is "formate-error" until we've finished parsing */
  ...
  */
  for (i = 0; i < dns->qdcount; i++) {
    ...
    if (dns->rr_count >= MAX_RRs)
      return;
    dns->rr_offset[dns->rr_count++] = offset;
  ...
  */
  for (i = 0; i < dns->ancount + dns->nscount; i++) {
    size_t rdlength;
    if (dns->rr_count >= ARRAY_SIZE(dns->rr_offset))
      return;
    dns->rr_offset[dns->rr_count++] = offset;
    offset = dns_name_skip(px, offset, max);
    offset += 10;
  ...
  dns->query_name.max_length = sizeof(dns->query_name_buffer);
...
unsigned handle_dns(struct Banner1 *banner1, struct Output *out,
  ...
  if (reason != 0) {
    output_report_banner(out, timestamp, &ip_them, 17, port_them,
                         ...
                         (const unsigned char *)reason, strlen(reason));
    return 0;
  }
