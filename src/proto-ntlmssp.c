...
void ntlmssp_decode(struct NtlmsspDecode *x, const unsigned char *px,
  ...
  name_offset = (size_t)(px[16] | px[17] << 8 | px[18] << 16 | px[19] << 24);
  ...
  info_offset = (size_t)(px[44] | px[45] << 8 | px[46] << 16 | px[47] << 24);

  /* Version field */
  {
    char buf[64];
    sprintf_s(buf, sizeof(buf), " version=%u.%u.%u ntlm-ver=%u", px[48], px[49],
              px[50] | (((unsigned)px[51]) << 8), px[55]);
