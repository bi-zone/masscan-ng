...
/*****************************************************************************
 *****************************************************************************/
static void ntp_modlist_parse(const unsigned char *px, size_t length,
  ...
  records_size = (size_t)record_count * record_size;
  if (4 + records_size > length) {
  ...
  {
    char msg[128];
    sprintf_s(msg, sizeof(msg), " response-size=%" PRIuPTR "-bytes more=%s",
              records_size, ((px[0] >> 6) & 1) ? "true" : "false");
...
/*****************************************************************************
 *****************************************************************************/
static void ntp_priv(const unsigned char *px, size_t length,
...
/*****************************************************************************
 *****************************************************************************/
static void ntp_v2_parse(const unsigned char *px, size_t length,
                         struct BannerOutput *banout, unsigned *request_id) {
...
unsigned ntp_handle_response(struct Banner1 *banner1, struct Output *out,
  ...
  size_t offset = parsed->app_offset;
  UNUSEDPARM(banner1);
  UNUSEDPARM(length);
  UNUSEDPARM(entropy);
