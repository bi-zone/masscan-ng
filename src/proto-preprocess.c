...
parse_ipv4 : {
  ...
  /* Check version */
  if ((px[offset] >> 4) != 4) {
    LOG(LEVEL_DEBUG_2, "[IPv4] not IPv4 or corrupt\n");
    return 0; /* not IPv4 or corrupt */
  }

  /* Check header length */
  header_length = (size_t)(px[offset] & 0x0F) * 4;
  ...
  fragment_offset = ((size_t)(ex16be(px + offset + 6) & 0x3FFF)) << 3;
  if (fragment_offset != 0 || (flags & 0x20)) {
    LOG(LEVEL_DEBUG_2, "[IPv4] fragmented\n");
    return 0; /* fragmented */
  }
  ...
  if (total_length < header_length) {
    LOG(LEVEL_DEBUG_2, "[IPv4] weird corruption\n");
  ...
  info->src_ip.ipv4 = (ipv4address_t)px[offset + 12] << 24 |
                      (ipv4address_t)px[offset + 13] << 16 |
                      (ipv4address_t)px[offset + 14] << 8 |
                      (ipv4address_t)px[offset + 15] << 0;
  info->src_ip.version = 4;
  info->dst_ip.ipv4 = (ipv4address_t)px[offset + 16] << 24 |
                      (ipv4address_t)px[offset + 17] << 16 |
                      (ipv4address_t)px[offset + 18] << 8 |
                      (ipv4address_t)px[offset + 19] << 0;
  ...
  if (info->ip_version != 4) {
    LOG(LEVEL_DEBUG_2, "[IPv4] ip_version != 4\n");
    return 0;
  }
  ...
  default:
    ...
    LOG(LEVEL_DEBUG_2,
        "[IPv4] TODO: should add more protocols, like ICMP. %u\n",
        info->ip_protocol);
...
parse_ipv6 : {
  ...
  /* Check version */
  if ((px[offset] >> 4) != 6) {
    LOG(LEVEL_DEBUG_2, "[IPv6] not IPv6 or corrupt\n");
    return 0; /* not IPv6 or corrupt */
  }
...
parse_ipv6_next:
  ...
  case 0x2c: /* IPv6 fragment */
    LOG(LEVEL_DEBUG_2, "[IPv6] fragmetn\n");
    return 0;
  default:
    LOG(LEVEL_DEBUG_2,
        "[IPv6] TODO: should add more protocols, like ICMP. %u\n",
        info->ip_protocol);
    return 0; /* TODO: should add more protocols, like ICMP */
  }
}
...
parse_vlanmpls:
  ...
  if (px[offset - 4 + 2] & 1) {
    ...
    LOG(LEVEL_DEBUG_2, "[vlanmpls] could not find final header\n");
    return 0;
  }
...
parse_wifi:
  ...
  case 0x88: /* QoS data */
    if (px[1] & 0x40) {
      LOG(LEVEL_DEBUG_2, "[parse_wifi] QoS data ???\n");
      return 0;
    }
    goto wifi_data;
    break;
  default:
    LOG(LEVEL_DEBUG_2, "[parse_wifi] ???\n");
    return 0;
  }
...
parse_radiotap_header:
   ...
   *   }; */
  {
    ...
    if (px[offset] != 0) {
      LOG(LEVEL_DEBUG_2, "[radiotap_header] it_version != 0\n");
      return 0;
    }
...
parse_prism_header:
   ...
   * this way. */
  {
    ...
    if (ex32le(px + offset + 0) != 0x00000044) {
      LOG(LEVEL_DEBUG_2, "[prism_header] ???\n");
      return 0;
    }
    header_length = (size_t)ex32le(px + offset + 4);
    if (header_length > 0xFFFFF) {
      LOG(LEVEL_DEBUG_2, "[prism_header] too big header\n");
      return 0;
    }
...
parse_ethertype:
  ...
  case 0x8847:
    goto parse_vlanmpls;
  default:
    LOG(LEVEL_DEBUG_2, "[ethertype] unknown 0x%x\n", ethertype);
    return 0;
  }
...
parse_linktype:
  ...
  default:
    LOG(LEVEL_DEBUG_2, "[linktype] unknown 0x%x\n", link_type);
    return 0;
  }
...
parse_arp:
  ...
  info->ip_offset = offset;
  {
    ...
    info->src_ip.version = 4;
    info->src_ip.ipv4 = (ipv4address_t)px[offset + hardware_length + 0] << 24 |
                        (ipv4address_t)px[offset + hardware_length + 1] << 16 |
                        (ipv4address_t)px[offset + hardware_length + 2] << 8 |
                        (ipv4address_t)px[offset + hardware_length + 3] << 0;
    info->dst_ip.version = 4;
    info->dst_ip.ipv4 =
        (ipv4address_t)px[offset + 2 * hardware_length + protocol_length + 0]
            << 24 |
        (ipv4address_t)px[offset + 2 * hardware_length + protocol_length + 1]
            << 16 |
        (ipv4address_t)px[offset + 2 * hardware_length + protocol_length + 2]
            << 8 |
        (ipv4address_t)px[offset + 2 * hardware_length + protocol_length + 3]
            << 0;
