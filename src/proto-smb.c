...
unsigned char smb2_negotiate_request[] = {
    ...
    0x00, 0x03, 0x02, 0x03};
unsigned char smb2_null_session_setup[] = {
...
/*****************************************************************************
 *****************************************************************************/
static const unsigned long long SMB_TICKS_PER_SECOND = 10000000LL;
static const unsigned long long SMB_EPOCH_DIFFERENCE = 11644473600LL;

static time_t convert_windows_time(long long int filetime) {
  unsigned long long seconds = filetime / SMB_TICKS_PER_SECOND;
  seconds -= SMB_EPOCH_DIFFERENCE;
  ...
  for (; offset < max; offset++)
    ...
    case N_BOOT8:
      smb->parms.negotiate2.boot_time |=
          (((uint64_t)px[offset]) << ((state - N_BOOT1) * 8));
      state++;
      break;
    case N_BLOB_OFFSET1:
...
static size_t smb2_parse_header(struct SMBSTUFF *smb, const unsigned char *px,
  ...
  for (; offset < max; offset++)
    ...
    case SMB2_MSGID8:
      smb->hdr.smb2.seqno |=
          (((uint64_t)px[offset]) << ((state - SMB2_MSGID1) * 8));
      state++;
      break;
    case SMB2_PID1:
    case SMB2_PID2:
    ...
    case SMB1_PARAMETERS:
      ...
      case 0x72:
        ...
        if (smb->hdr.smb1.flags2 & 0x0800) {
          tcp_transmit(more, smb1_null_session_setup_ex,
          ...
          if (smb->parms.negotiate.SessionKey) {
            ...
            tcp_transmit(more, buf, sizeof(smb1_null_session_setup),
                         TCPTRAN_DYNAMIC);

            /* NOTE: the following line is here to silence LLVM warnings about a
      ...
      if (smb->hdr.smb2.offset >= smb->hdr.smb2.struct_length) {
        ...
        case 0x00: /* negotiate response */
          if (smb->hdr.smb2.seqno == 0) {
            tcp_transmit(more, smb2_negotiate_request,
                         sizeof(smb2_negotiate_request), 0);
          } else if (smb->hdr.smb2.seqno == 1) {
            tcp_transmit(more, smb2_null_session_setup,
 ...
 *****************************************************************************/
static void
smb_parse_record(const struct Banner1 *banner1, void *banner1_private,
  ...
  assert(pstate->parser_stream == &banner_smb0 ||
         pstate->parser_stream == &banner_smb1);
...
static void smb_cleanup(struct Banner1 *banner1) {
  UNUSEDPARM(banner1);
  return;
}
...
#if 0
    {
        ...
        for (i=2; i< 5 && i<sizeof(packet_bytes); i++) {
            ...
            for (j=0; j<8; j++) {
				...
				state->app_proto = PROTO_SMB;
				state->parser_stream = &banner_smb1;
				init_application_proto(banner1, state, banout1);
				application_receive_next(
					banner1, state, state, packet_bytes,
					sizeof(packet_bytes), banout1, &more);
				free_interactive_data(&more);
				cleanup_application_proto(banner1, state);
                banner1_destroy(banner1);
                banout_release(banout1);
...
/*****************************************************************************
 *****************************************************************************/
static void smb_transmit_cleanup(const struct Banner1 *banner1,
                                 struct ProtocolState *pstate,
                                 struct ResendPayload *resend_payload) {
