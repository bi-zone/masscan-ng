...
/****************************************************************************
 ****************************************************************************/
static size_t next_id(const unsigned char *oid, size_t *offset,
                      size_t oid_length) {
  size_t result = 0;
...
static void snmp_banner_oid(const unsigned char *oid, size_t oid_length,
  ...
  /* Do remaining OIDs */
  for (i = found_offset; i < oid_length;) {
    char foo[32] = {0};
    size_t x = next_id(oid, &i, oid_length);

    if (x == 0 && i >= oid_length)
      break;

    sprintf_s(foo, sizeof(foo), ".%" PRIuPTR "", x);
