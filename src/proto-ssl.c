...
static void BANNER_VERSION(struct BannerOutput *banout, unsigned version_major,
  ...
  case 0x0304:
    ...
    break;
  default:
    sprintf_s(foo, sizeof(foo), "SSLver[%u,%u] ", version_major, version_minor);
...
static void parse_server_hello(const struct Banner1 *banner1,
  ...
  size_t remaining = hello->remaining;
  size_t i;
  enum {
    VERSION_MAJOR,
    VERSION_MINOR,
...
static void parse_server_cert(const struct Banner1 *banner1,
  ...
  size_t cert_remaining = data->sub.remaining;
  size_t i;
  enum {
    LEN0,
    LEN1,
  ...
  for (i = 0; i < length; i++)
    ...
    case CERT: {
      ...
      if (banner1->is_capture_cert) {
        banout_append_base64(banout, PROTO_X509_CERT, px + i, len,
                             &pstate->base64);
      }
      ...
      if (cert_remaining == 0) {
         ...
         * a record of it */
        if (banner1->is_capture_cert) {
          banout_finalize_base64(banout, PROTO_X509_CERT, &pstate->base64);
...
static void parse_handshake(const struct Banner1 *banner1,
                            void *banner1_private, struct ProtocolState *pstate,
                            const unsigned char *px, size_t length,
                            struct BannerOutput *banout,
                            struct InteractiveData *more) {
  struct SSLRECORD *ssl = &pstate->sub.ssl;
  unsigned state = ssl->handshake.state;
  size_t remaining = ssl->handshake.remaining;
  size_t i;
  enum {
    START,
    LENGTH0,
...
static void parse_alert(const struct Banner1 *banner1, void *banner1_private,
                        struct ProtocolState *pstate, const unsigned char *px,
...
ssl_parse_record(const struct Banner1 *banner1, void *banner1_private,
  ...
  assert(pstate->parser_stream == &banner_ssl);
  ...
  for (i = 0; i < length; i++)
    ...
    case UNKNOWN:
      switch_application_proto(banner1, pstate, resend_payload, banout, keyout,
                               PROTO_NONE, NULL);
      application_receive_next(banner1, pstate, pstate, resend_payload, px,
                               length, banout, signout, keyout, more);
      return;
    default:
      i = length;
    }
...
static char ssl_hello_template[] =
    ...
    "\x52\x48\xc5\x1a\x23\xf7\x3a\x4e\xdf\xe2\xb4\x82"
    "\x2f\xff\x09\x54" /* random
                        */
    "\x9f\xa7\xc4\x79\xb0\x68\xc6\x13\x8c\xa4\x1c\x3d"
    "\x22\xe1\x1a\x98" /* TODO:
                          re-randomize
                          for
                          each
                          request,
    ...
    "\x84\xb4\x2c\x85\xaf\x6e\xe3\x59\xbb\x62\x68\x6c"
    "\xff\x28\x3d\x27" /* random
                        */
    "\x3a\xa9\x82\xd9\x6f\xc8\xa2\xd7\x93\x98\xb4\xef"
    "\x80\xe5\xb9\x90" /* TODO:
                          re-randomize
                          for
                          each
                          request,
    ...
    "\xc0\x0a\xc0\x14\x00\x39\x00\x6b\x00\x35\x00\x3d"
    "\xc0\x07\xc0\x09"
    "\xc0\x23\xc0\x11\xc0\x13\xc0\x27\x00\x33\x00\x67"
    "\x00\x32\x00\x05"
    ...
    "\xef\x00"
    "\x00\x1a"
    "\x00\x18\x00\x00\x15\x73\x79\x6e\x64\x69\x63\x61"
    "\x74\x69\x6f\x6e"
    ...
    "\x00\x23"
    "\x00\xb0"
    "\x81\x01\x19\x67\x60\x1e\x04\x42\x9a\xf3\xe2\x3c"
    "\x86\x58\x4f\x87"
    "\x69\x44\xb0\x1d\x8e\x01\xfa\xa5\x87\x3d\x5d\xdc"
    "\x16\x4c\xb4\x20"
    "\xda\xd3\x42\xb0\x88\xec\x0a\x13\xc3\xc6\x4c\x44"
    "\x74\x7d\xf5\x83"
    "\x93\xeb\x16\x60\x7e\x47\x07\x15\xae\x68\x3f\x32"
    "\xfc\x28\x71\xdd"
    "\x8d\x2a\xe0\x9e\x03\xad\x28\xd9\x89\x2f\x0f\x07"
    "\xaf\xc1\x27\x8e"
    "\xf1\x57\xfb\xc6\xc4\xd4\x56\x3a\xf6\xed\x59\x61"
    "\x4a\x17\x14\x0b"
    "\xd7\x7c\xae\xfe\x55\xd9\x7a\xa6\xf6\xc6\x57\xb5"
    "\x3c\xed\x78\x9d"
    "\xee\x39\xd8\x67\x02\x09\x92\xcb\xa5\x66\xa3\x48"
    "\x3d\x06\xed\xa5"
    "\x02\x2e\x9b\x16\xf6\x2b\xe7\x3f\x79\x65\x1a\xcb"
    "\x6c\x5c\xbd\x6b"
    "\xad\x11\xde\xbe\xdf\x35\xdb\x0b\xff\x2c\x90\x94"
    "\x32\xb5\x94\x57"
    "\x3d\x5e\x25\xd2\x1b\xd2\x44\x85\x96\x31\x28\x69"
    "\xd7\x4a\x13\x0a"
    "\x33\x74\x00\x00\x75\x4f\x00\x00\x00\x05\x00\x05"
    "\x01\x00\x00\x00"
    "\x00";
...
static char *ssl_add_cipherspec_sslv3(void *templ, unsigned cipher_spec,
  ...
  size_px = ssl_hello_size(templ) + 2;
  px = REALLOC(templ, size_px);
...
char *ssl_add_cipherspec(void *templ, unsigned cipher_spec,
  ...
  if (px[0] != 0x16) {
    LOG(LEVEL_WARNING, "internal error\n");
    return templ;
  }
 ...
 *****************************************************************************/
size_t ssl_hello_size(const void *templ) {
  const unsigned char *px = (const unsigned char *)templ;
  size_t template_size;

  template_size = (((size_t)px[3]) << 8 | ((size_t)px[4])) + 5;
...
static int ssl_selftest(void) {
  ...
  state->parser_stream = &banner_ssl;
  init_application_proto(banner1, state, &resend_payload, banout1, &keyout);
  {
    ...
    for (i = 0; i < ssl_test_case_3_size; i++) {
      application_receive_next(banner1, state, state, &resend_payload,
                               ssl_test_case_3 + i, 1, banout1, signout,
                               &keyout, &more);
  ...
  for (ii = 0; ii < ssl_test_case_3_size; ii++) {
    application_receive_next(banner1, state, state, &resend_payload,
                             (const unsigned char *)ssl_test_case_3 + ii, 1,
                             banout1, signout, &keyout, &more);
    free_interactive_data(&more);
  }
  cleanup_application_proto(banner1, state, &resend_payload);
  keyout_release(&keyout);
