...
static void rdp_parse(const struct Banner1 *banner1, void *banner1_private,
  ...
  size_t state = pstate->state & 0xFFFFFF;
  ...
  assert(pstate->parser_stream == &banner_rdp);
...
static void rdp_cleanup(struct Banner1 *banner1) {
  UNUSEDPARM(banner1);
  return;
}
...
static int rdp_selftest_item(const char *input, size_t length,
  ...
  pstate->app_proto = PROTO_RDP;
  pstate->parser_stream = &banner_rdp;
  ...
  application_receive_next(banner1, pstate, pstate, resend_payload,
                           (const unsigned char *)input, length, banout1,
                           signout, &keyout, &more);
