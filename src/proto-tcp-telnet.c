...
static const char *option_name_lookup(unsigned optnum) {
  size_t i;
...
static void telnet_parse(const struct Banner1 *banner1, void *banner1_private,
  ...
  assert(pstate->parser_stream == &banner_telnet);
  ...
  for (offset = 0; offset < length; offset++) {
    ...
    case TELNET_IAC:
      ...
      case 243: /* 0xF3 BRK - NVT character BRK. */
        banout_append(banout, PROTO_TELNET, " IAC(BRK)", AUTO_LEN);
        state = 0;
        break;
    ...
    case TELNET_WONT:
      ...
      }
      {
        ...
        if (name[0]) {
          banout_append_char(banout, PROTO_TELNET, ' ');
          banout_append(banout, PROTO_TELNET, foobar[state - TELNET_DO],
                        AUTO_LEN);
          banout_append_char(banout, PROTO_TELNET, '(');
          banout_append(banout, PROTO_TELNET, name, AUTO_LEN);
          banout_append_char(banout, PROTO_TELNET, ')');
...
#define r_length (256 * 3 * 4)
    ...
    if (r_offset) {
      ...
      tcp_transmit(more, outbuf, r_offset, TCPTRAN_DYNAMIC);
    }
  }
  pstate->state = state;
}
...
static void telnet_cleanup(struct Banner1 *banner1) {
  UNUSEDPARM(banner1);
  return;
}
...
static int telnet_selftest_item(const char *input, const char *output) {
  ...
  pstate->app_proto = PROTO_TELNET;
  pstate->parser_stream = &banner_telnet;
  init_application_proto(banner1, pstate, resend_payload, banout1, &keyout);
  /* Parse the input payload */
  application_receive_next(banner1, pstate, pstate, resend_payload,
                           (const unsigned char *)input, strlen(input), banout1,
                           signout, &keyout, &more);
  free_interactive_data(&more);
  // LOG(LEVEL_ERROR, "%.*s\n", (int)banout_string_length(banout1,
...
static int telnet_selftest(void) {
  ...
  for (i = 0; tests[i].input; i++) {
    ...
    if (err) {
      LOG(LEVEL_ERROR, "telnet: selftest fail, item %" PRIu64 "\n", i);
      return err;
    }
  }
  return 0;
