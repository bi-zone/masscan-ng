...
int rawsock_get_default_interface(char *ifname, size_t sizeof_ifname) {
  ...
  if (fd < 0) {
    LOG(LEVEL_ERROR, "socket(PF_ROUTE): %s\n", strerror(errno));
    free(rtm);
    return errno;
  }
  LOG(LEVEL_DEBUG, "[+] getif: got socket handle\n");
   ...
   * response that will never arrive */
  {
    ...
    err = setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,
                     sizeof(timeout));
    if (err < 0)
      LOG(LEVEL_WARNING, "[-] SO_RCVTIMEO: %d %s\n", errno, strerror(errno));

    err = setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout,
                     sizeof(timeout));
    if (err < 0)
      LOG(LEVEL_WARNING, "[-] SO_SNDTIMEO: %d %s\n", errno, strerror(errno));
  }
  ...
  for (;;) {
    ...
    LOG(LEVEL_DEBUG, "[+] getif: got response, len=%d\n", err);

    if (rtm->rtm_seq != seq) {
      LOG(LEVEL_WARNING, "seq: %u %u\n", rtm->rtm_seq, seq);
      continue;
    }
    if (rtm->rtm_pid != getpid()) {
      LOG(LEVEL_WARNING, "pid: %u %u\n", rtm->rtm_pid, getpid());
      continue;
    }
    break;
  }
...
static int read_netlink(int fd, char *bufPtr, size_t sizeof_buffer, int seqNum,
  ...
  do {
    ...
    if ((NLMSG_OK(nlHdr, readLen) == 0) || (nlHdr->nlmsg_type == NLMSG_ERROR)) {
      LOG(LEVEL_ERROR, "Error in received packet: %s\n", strerror(errno));
      return -1;
    }
...
#define FORMATADDR(n)                                                          \
  ...
  for (; RTA_OK(rtAttr, rtLen); rtAttr = RTA_NEXT(rtAttr, rtLen)) {
    ...
    case RTA_OIF:
      ...
      // LOG(LEVEL_DEBUG_2, "ifname=%s ", rtInfo->ifName);
      break;
    case RTA_GATEWAY:
      rtInfo->gateWay.s_addr = *(u_int *)RTA_DATA(rtAttr);
      // LOG(LEVEL_DEBUG_2, "gw=%u.%u.%u.%u ",
      // FORMATADDR(rtInfo->gateWay.s_addr));
      break;
    case RTA_PREFSRC:
      rtInfo->srcAddr.s_addr = *(u_int *)RTA_DATA(rtAttr);
      // LOG(LEVEL_DEBUG_2, "src=%u.%u.%u.%u ",
      // FORMATADDR(rtInfo->srcAddr.s_addr));
      break;
    case RTA_DST:
      rtInfo->dstAddr.s_addr = *(u_int *)RTA_DATA(rtAttr);
      // LOG(LEVEL_DEBUG_2, "dst=%u.%u.%u.%u ",
      // FORMATADDR(rtInfo->dstAddr.s_addr));
      break;
    case RTA_PRIORITY:
      rtInfo->priority = *(int *)RTA_DATA(rtAttr);
      // LOG(LEVEL_DEBUG_2, "priority=0x%08x ", rtInfo->priority);
      break;
    default:
        // LOG(LEVEL_DEBUG_2, "rta_type=%d ", rtAttr->rta_type)
        ;
    }
  }
  // LOG(LEVEL_DEBUG_2, "\n");
...
int rawsock_get_default_interface(char *ifname, size_t sizeof_ifname) {
  ...
  for (; NLMSG_OK(nlMsg, len); nlMsg = NLMSG_NEXT(nlMsg, len)) {
    ...
    // LOG(LEVEL_DEBUG_1, "if: nlmsg_type=%d nlmsg_flags=0x%x\n",
...
again:
  ...
  for (pAdapter = pAdapterInfo; pAdapter; pAdapter = pAdapter->Next) {
     ...
     * we'll use that one */
    if (ipv4) {
      sprintf_s(ifname, sizeof_ifname, "\\Device\\NPF_%s",
