...
again:
  ...
  if (bytes_read < 16) {
    LOG(LEVEL_WARNING, "%s: failed to read header\n", capfile->filename);
    if (bytes_read == 0 || ferror(capfile->fp) == 0) {
      LOG(LEVEL_WARNING, "%s: %s\n", capfile->filename, strerror(errno));
    } else {
  ...
  *r_original_length = PCAP32(byte_order, header + 12);
  ...
  bytes_read = fread(buf, 1, (size_t)(*r_captured_length), capfile->fp);
  if (bytes_read < (size_t)(*r_captured_length)) {
    if (bytes_read <= 0) {
...
void pcapfile_get_timestamps(struct PcapFile *capfile, time_t *start,
                             time_t *end) {
  *start = (time_t)capfile->start_sec;
  *end = (time_t)capfile->end_sec;
}
...
struct PcapFile *pcapfile_openread(const char *capfilename) {
  ...
  case 0xd4c3b2a1:
    ...
    break;
  default:
    LOG(LEVEL_WARNING, "%s: unknown byte-order in cap file\n", capfilename);
  ...
  /* Version (of the libpcap standard) */
  {
    ...
    if (major != 2 || minor != 4)
      LOG(LEVEL_WARNING, "%s: unknown version %d.%d\n", capfilename, major,
          minor);
  }
   ...
   */
  {
    ...
    capfile = (struct PcapFile *)calloc(1, sizeof(*capfile));
    if (capfile == NULL)
      exit(1);
...
struct PcapFile *pcapfile_openappend(const char *capfilename,
  ...
  if (fp == NULL) {
    LOG(LEVEL_WARNING, "Could not open capture file to append frame\n");
    LOG(LEVEL_WARNING, "%s: %s\n", capfilename, strerror(errno));
  ...
  if (fread(buf, 1, 24, fp) != 24) {
    LOG(LEVEL_WARNING, "Error reading capture file header\n");
    LOG(LEVEL_WARNING, "%s: %s\n", capfilename, strerror(errno));
    fclose(fp);
    return pcapfile_openappend(capfilename, linktype);
  }
  ...
  /* Version */
  {
    ...
    if (major != 2 || minor != 4) {
      LOG(LEVEL_WARNING, "%s: unknown version %u.%u\n", capfilename, major,
          minor);
    }
  }
  ...
  if (linktype != file_linktype) {
    ...
    char *dot_in_capfilename;
    size_t i;

    fclose(fp);

    linkspec_length =
        snprintf(linkspec, sizeof(linkspec), "-linktype%u", linktype);
    ...
    dot_in_capfilename = strchr(capfilename, '.');
    if (dot_in_capfilename)
      i = dot_in_capfilename - capfilename;
    else
...
void pcapfile_writeframe(struct PcapFile *capfile, const void *buffer,
  ...
  if (fwrite(buffer, 1, (size_t)buffer_size, capfile->fp) !=
      (size_t)buffer_size) {
