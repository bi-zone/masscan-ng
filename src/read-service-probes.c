...
static struct RangeList parse_ports(struct NmapServiceProbeList *list,
  ...
  if (is_error) {
    LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIu64 ": bad port spec\n",
        list->filename, list->line_number, p - line);
...
static unsigned parse_number(struct NmapServiceProbeList *list,
  ...
  while (offset < line_length && isdigit((int)line[offset])) {
    number = number * 10;
  ...
  if (offset != line_length) {
    LOG(LEVEL_WARNING,
        "%s:%" PRIuPTR ":%" PRIuPTR ": unexpected character '%c'\n",
        list->filename, list->line_number, offset,
        isprint((int)line[offset]) ? line[offset] : '.');
  }
...
static char *parse_name(const char *line, size_t *r_offset,
  ...
  while (*r_offset < line_length && !isspace((int)line[*r_offset]))
    (*r_offset)++;
...
parse_fallback(struct NmapServiceProbeList *list, const char *line,
  ...
  while (offset < line_length) {
    ...
    name_length = offset - name_offset;
    while (offset < line_length &&
           (isspace((int)line[offset]) || line[offset] == ','))
      offset++; /* trim trailing whitespace */
    if (name_length == 0) {
      LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":" PRIu64 ": name too short\n",
          list->filename, list->line_number, name_offset);
      break;
    }
...
static void parse_probe(struct NmapServiceProbeList *list, const char *line,
  ...
  size_t line_number = list->line_number;
  ...
  if (line_length - offset <= 3) {
    LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIuPTR ": line too short\n",
  ...
  if (memcmp(line + offset, "TCP", 3) == 0)
    ...
    LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIuPTR ": unknown protocol\n",
        filename, line_number, offset);
    goto parse_error;
  }
  offset += 3;
  if (!isspace((int)line[offset])) {
    LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIuPTR ": unexpected character\n",
        filename, line_number, offset);
    goto parse_error;
  }
  while (offset < line_length && isspace((int)line[offset]))
  ...
  if (probe->name == 0) {
    LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIuPTR ": probename parse error\n",
        filename, line_number, offset);
    goto parse_error;
  }
   ...
   */
  {
    ...
    if (line[offset++] != 'q') {
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR ": expected 'q', found '%c'\n", filename,
          line_number, offset,
          isprint((int)line[offset - 1]) ? line[offset - 1] : '.');
      goto parse_error;
    }
    ...
    while (offset < line_length && line[offset] != delimiter) {
      ...
      if (offset >= line_length || line[offset] == delimiter) {
        LOG(LEVEL_WARNING,
            "%s:%" PRIuPTR ":%" PRIuPTR ": premature end of field\n", filename,
            line_number, offset);
        goto parse_error;
      }
      ...
      switch (line[offset++]) {
      default:
        LOG(LEVEL_WARNING, "%s:%" PRIuPTR ": %.*s\n", filename, line_number,
            (unsigned)line_length, line);
        LOG(LEVEL_WARNING,
            "%s:%" PRIuPTR ":%" PRIuPTR ": unexpected escape character '%c'\n",
            filename, line_number, offset - 1,
            isprint((int)line[offset - 1]) ? line[offset - 1] : '.');
        goto parse_error;
      case '\\':
        x[x_offset++] = '\\';
      ...
      case 'x':
        ...
        if (!is_hexchar(line[offset + 0]) || !is_hexchar(line[offset + 1])) {
          LOG(LEVEL_WARNING,
              "%s:%" PRIuPTR ":%" PRIuPTR ": expected hex, found '%c%c'\n",
              filename, line_number, offset,
              isprint((int)line[offset + 1]) ? line[offset + 1] : '.',
              isprint((int)line[offset + 2]) ? line[offset + 2] : '.');
          goto parse_error;
        }
    ...
    if (offset >= line_length || line[offset] != delimiter) {
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR ": missing end delimiter '%c'\n",
          filename, line_number, offset,
          isprint((int)delimiter) ? delimiter : '.');
      goto parse_error;
    }
    // offset++;
  }
...
static struct ServiceProbeMatch *parse_match(struct NmapServiceProbeList *list,
  ...
  if (match->service == 0) {
    LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIuPTR ": servicename is empty\n",
        filename, line_number, offset);
    goto parse_error;
  }
   ...
   */
  {
    ...
    if (line[offset] != 'm') {
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR ": expected 'm', found '%c'\n", filename,
          line_number, offset, isprint((int)line[offset]) ? line[offset] : '.');
      goto parse_error;
    }
    offset++;
    ...
    if (offset >= line_length || line[offset] != delimiter) {
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR ": missing ending delimiter '%c'\n",
          filename, line_number, offset,
          isprint((int)delimiter) ? delimiter : '.');
      goto parse_error;
    } else {
    ...
    while (offset < line_length && !isspace((int)line[offset])) {
      ...
      default:
        LOG(LEVEL_WARNING,
            "%s:%" PRIuPTR ":%" PRIuPTR ": unknown regex pattern option '%c'\n",
            filename, line_number, offset,
            isprint((int)line[offset]) ? line[offset] : '.');
        goto parse_error;
      }
  ...
  while (offset < line_length) {
    ...
    if (offset + 2 >= line_length) {
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR
          ": unexpected character at end of line '%c'\n",
          filename, line_number, offset,
          isprint((int)line[offset]) ? line[offset] : '.');
      goto parse_error;
    }
    ...
    if (id == 'c') {
      ...
      if (memcmp(line + offset, "pe:", 3) != 0) {
        LOG(LEVEL_WARNING,
            "%s:%" PRIuPTR ":%" PRIuPTR ": expected string 'cpe:'\n", filename,
            line_number, offset);
        goto parse_error;
      }
      offset += 3;
    ...
    default:
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR ": versioninfo unknown identifier '%c'\n",
          filename, line_number, offset, isprint((int)id) ? id : '.');
      goto parse_error;
    }
    ...
    if (offset < line_length && !isspace((int)line[offset])) {
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR
          ": unexpected character after delimiter '%c'\n",
          filename, line_number, offset,
          isprint((int)delimiter) ? delimiter : '.');
      goto parse_error;
    }
...
static void parse_line(struct NmapServiceProbeList *list, const char *line) {
  ...
  while (offset && isspace((int)line[offset]))
    offset++;
  while (line_length && isspace((int)line[line_length - 1]))
    line_length--;

  /* Ignore comment lines */
  if (ispunct((int)line[offset]))
    return;
  ...
  switch ((int)type) {
  case SvcP_Unknown:
    LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIuPTR ": unknown type: '%.*s'\n",
        filename, line_number, offset, (int)offset - 0, line);
    return;
  case SvcP_Exclude:
    if (list->count) {
      ...
      LOG(LEVEL_WARNING,
          "%s:%" PRIuPTR ":%" PRIuPTR
          ": 'Exclude' directive only valid before any 'Probe'\n",
          filename, line_number, offset);
    } else {
      ranges = parse_ports(list, line, offset, line_length);
      if (ranges.count == 0) {
        LOG(LEVEL_WARNING,
            "%s:%" PRIuPTR ":%" PRIuPTR ": 'Exclude' bad format\n", filename,
            line_number, offset);
      } else {
  ...
  if (list->count == 0) {
    LOG(LEVEL_WARNING,
        "%s:%" PRIuPTR ":%" PRIuPTR ": 'directive only valid after a 'Probe'\n",
        filename, line_number, offset);
    return;
  }
  ...
  case SvcP_Ports:
    ranges = parse_ports(list, line, offset, line_length);
    if (ranges.count == 0) {
      LOG(LEVEL_WARNING, "%s:%" PRIuPTR ":%" PRIuPTR ": bad ports format\n",
          filename, line_number, offset);
    } else {
...
struct NmapServiceProbeList *nmapserviceprobes_read_file(const char *filename) {
  ...
  fclose(fp);
  result->filename = NULL; /* name no longer valid after this point */
  result->line_number =
      ~((size_t)0); /* line number no longer valid after this point */
...
static void nmapserviceprobes_print_ports(const struct RangeList *ranges,
  ...
  for (i = 0; i < ranges->count; i++) {
    ...
    if (Templ_TCP <= begin && begin <= Templ_TCP_last) {
      proto = Templ_TCP;
    } else if (Templ_UDP <= begin && begin <= Templ_UDP_last) {
      proto = Templ_UDP;
    } else if (Templ_SCTP <= begin && begin <= Templ_SCTP_last) {
      proto = Templ_SCTP;
    } else if (Templ_ICMP <= begin && begin <= Templ_ICMP_last) {
      if (begin == Templ_ICMP_echo || begin == Templ_ICMP_timestamp) {
        proto = begin;
      } else {
        proto = Templ_ICMP_last;
      }
    } else if (Templ_ARP <= begin && begin <= Templ_ARP_last) {
      proto = Templ_ARP;
    } else if (Templ_Oproto <= begin && begin <= Templ_Oproto_last) {
      proto = Templ_Oproto;
    } else {
      proto = Templ_VulnCheck;
    }
...
void nmapserviceprobes_free(struct NmapServiceProbeList *list) {
  ...
  rangelist_remove_all(&list->exclude);
  if (list->list) {
    for (i = 0; i < list->count; i++) {
