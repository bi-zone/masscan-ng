...
#define BASE_STATE ((size_t)(0))
#define UNANCHORED_STATE ((size_t)(1))
 ...
 ****************************************************************************/
struct SmackRow {
  size_t m_next_state[ALPHABET_SIZE];
  size_t m_fail_state;
};
...
struct SMACK {
  ...
  size_t m_pattern_count;
  size_t m_pattern_max;
...
struct SMACK *smack_create(const char *name, unsigned nocase) {
  ...
  smack->name = (char *)malloc(size_name + 1);
  if (smack->name == NULL) {
    LOG(LEVEL_ERROR, "%s: out of memory error\n", "smack");
    exit(1);
  }

  memcpy(smack->name, name, size_name + 1);
  return smack;
}

/****************************************************************************
 ****************************************************************************/
static void create_intermediate_table(struct SMACK *smack, size_t size) {
  struct SmackRow *x;
...
/****************************************************************************
 ****************************************************************************/
static void create_matches_table(struct SMACK *smack, size_t size) {
 ...
 * from the new list.
 ****************************************************************************/
static unsigned id_already_exists(const size_t *ids, size_t count,
                                  size_t new_id) {
...
void smack_add_pattern(struct SMACK *smack, const void *v_pattern,
                       size_t pattern_length, size_t id, unsigned flags) {
  ...
  if (smack->m_pattern_count + 1 >= smack->m_pattern_max) {
    ...
    size_new_list = sizeof(*new_list) * new_max;
    new_list = (struct SmackPattern **)malloc(size_new_list);
    if (new_list == NULL) {
      LOG(LEVEL_ERROR, "%s: out of memory error\n", "smack");
      exit(1);
    }
...
static void smack_add_prefixes(struct SMACK *smack, struct SmackPattern *pat) {
  ...
  for (; i < pattern_length; i++) {
    size_t new_state = smack->m_state_count++;
    if (pat->is_snmp_hack)
...
static void smack_stage0_compile_prefixes(struct SMACK *smack) {
  ...
  if (smack->is_anchor_begin) {
    size_t anchor_begin = smack->m_state_count++;
...
static void smack_stage4_make_final_table(struct SMACK *smack) {
  size_t row;
  size_t row_count = smack->m_state_count;
  size_t column_count;
  transition_t *table;
  size_t size_table;
  ...
  column_count = ((size_t)1) << smack->row_shift;

  /* Allocate table: rows*columns */
  size_table = sizeof(transition_t) * row_count * column_count;
  table = malloc(size_table);
  if (table == NULL) {
    LOG(LEVEL_ERROR, "%s: out of memory error\n", "smack");
    exit(1);
  }
  memset(table, 0, size_table);
...
/****************************************************************************
 ****************************************************************************/
static void swap_rows(struct SMACK *smack, size_t row0, size_t row1) {
...
static void smack_stage3_sort(struct SMACK *smack) {
  size_t start = 0;
  size_t end = smack->m_state_count;
...
static void smack_fixup_wildcards(struct SMACK *smack) {
  ...
  for (i = 0; i < smack->m_pattern_count; i++) {
    ...
    for (j = 0; j < pat->pattern_length; j++) {
      ...
      while (offset < j)
        smack_search_next(smack, &row, pat->pattern, &offset, j);
...
#else
      UNUSEDPARM(row_size);
      UNUSEDPARM(base_state);
      UNUSEDPARM(next_pattern);
#endif
    }
  }
}
...
void smack_compile(struct SMACK *smack) {
  ...
  if (smack->is_nocase) {
    for (c = 'A'; c <= 'Z'; c++) {
      smack->char_to_symbol[c] = smack->char_to_symbol[tolower(c)];
    }
  }
  ...
  for (i = 0; i < smack->m_pattern_count; i++) {
    ...
    smack->m_state_max += (size_t)pat->is_anchor_begin;
    smack->m_state_max += (size_t)pat->is_anchor_end;
  }
 ...
 ****************************************************************************/
static size_t handle_match(struct SMACK *smack, size_t index,
                           int (*callback_function)(size_t id, size_t index,
                                                    void *callback_data),
                           void *callback_data, size_t state) {
...
/****************************************************************************
 ****************************************************************************/
size_t smack_search(struct SMACK *smack, const void *v_px, size_t length,
  ...
  for (i = 0; i < length; i++) {
    ...
    if (print_transitions) {
      LOG(LEVEL_ERROR, "%s+%c = %s%s\n", smack->m_match[row].DEBUG_name, c,
          smack->m_match[*(table + (row << row_shift) + (size_t)column)]
              .DEBUG_name,
          smack->m_match[*(table + (row << row_shift) + (size_t)column)].m_count
              ? "$$"
              : "");
      print_transitions--;
    }
...
static size_t inner_match(const unsigned char *px, size_t length,
                          ...
                          const transition_t *table, size_t *state,
                          size_t match_limit, unsigned row_shift) {
...
/*****************************************************************************
 *****************************************************************************/
size_t smack_search_next(struct SMACK *smack, size_t *current_state,
                         const void *v_px, size_t *offset, size_t length) {
  ...
  register size_t match_limit = smack->m_match_limit;
...
/****************************************************************************
 ****************************************************************************/
size_t smack_next_match(struct SMACK *smack, size_t *current_state) {
  size_t row, current_matches;
  size_t id = SMACK_NOT_FOUND;
...
/****************************************************************************
 ****************************************************************************/
size_t smack_search_end(struct SMACK *smack, FOUND_CALLBACK cb_found,
                        void *callback_data, size_t *current_state) {

  size_t found_count = 0;
  transition_t *table = smack->table;
  unsigned row_shift = smack->row_shift;
  size_t row = *current_state;
  const struct SmackMatches *match = smack->m_match;
...
size_t smack_search_next_end(struct SMACK *smack, size_t *current_state) {

  transition_t *table = smack->table;
  unsigned row_shift = smack->row_shift;
  size_t row = *current_state & 0xFFFFFF;
  size_t current_matches = (*current_state) >> 24;
  const struct SmackMatches *match = smack->m_match;
  unsigned column = smack->char_to_symbol[CHAR_ANCHOR_END];
  size_t id = SMACK_NOT_FOUND;
...
/*static void print_smack(struct SMACK *s) {
        size_t i;
        size_t j;
        ...
        for(i = 0; i < s->m_state_count; i++) {
                printf("state: %3" PRIuPTR " -", i);
                for(j = 0; j < s->m_match[i].m_count; j++) {
                        printf(" %3" PRIuPTR "", s->m_match[i].m_ids[j]);
                }
                printf("\n");
        }
        printf("\n");
        ...
        printf("\n");
        for(i = 0; i < s->m_state_count; i++) {
                printf("state: %3"PRIuPTR" - %3"PRIuPTR"", i,
s->m_state_table[i].m_fail_state); for(j = 0; j <= s->symbol_count; j++) {
                        unsigned c = s->symbol_to_char[j];
                        size_t next_satate =
s->m_state_table[i].m_next_state[c]; if(next_satate == FAIL) { printf("   -");
                        } else {
                                printf(" %3"PRIuPTR"", next_satate);
                        }
                }
                printf("\n");
        }
        ...
        if(s->table == NULL) {
                ...
                for(i = 0; i < s->m_state_count; i++) {
                        printf("state: %3"PRIuPTR"", i);
                        for(j = 0; j <= s->symbol_count; j++) {
                                size_t next_satate = *(s->table + (i <<
s->row_shift) + j); if(next_satate == FAIL) { printf("   -"); } else { printf("
%3"PRIuPTR"", next_satate);
                                }
                        }
                        printf("\n");
                }
...
int smack_benchmark(void) {
  ...
  if (buf == NULL) {
    LOG(LEVEL_ERROR, "Benchmark error. Failed alloc memory\n");
    smack_destroy(s);
    return 1;
  }
  ...
  if (result) {
    ...
    double rate = ((double)BUF_SIZE * (double)ITERATIONS * 8.0) / elapsed;
    double cycles =
        ((double)BUF_SIZE * (double)ITERATIONS) / ((double)(cycle2 - cycle1));
...
#define TEST(pat, offset, str)                                                 \
  if ((pat) != id || (offset) != i) {                                          \
    return 1 + LOG(LEVEL_ERROR,                                                \
                   "[-] smack: fail %s %s, id = %" PRIdPTR ", "                \
                   "offset = %" PRIuPTR " on line %u\n",                       \
                   s->name, (str), id, i, __LINE__);                           \
  }

#define TEST_END(pat, str)                                                     \
  if ((pat) != id) {                                                           \
    return 1 + LOG(LEVEL_ERROR,                                                \
                   "[-] smack: fail %s %s, id = %" PRIdPTR " on line %u\n",    \
                   s->name, (str), id, __LINE__);                              \
  }

  {
    ...
    size_t i;
    const char *text =
        "ahpropfinddf;orderpatchposearchmoversion-controlockasldhf";
    ...
    for (i = 0; patterns[i]; i++) {
      smack_add_pattern(s, patterns[i], strlen(patterns[i]), i, 0);
    }
    ...
    if (id != END_TEST_THINGY1 && id != END_TEST_THINGY2) {
       ...
       */
      LOG(LEVEL_ERROR,
          "[-] smack: fail %s \"dhf\" or \"ldhf\", id = %" PRId64 ", "
          "offset = %" PRId64 " on line %u\n",
          s->name, id, i, __LINE__);
      smack_destroy(s);
      return 1;
    }
  ...
  {
    struct SMACK *s;
    const char *patterns[] = {"BEGIN", "END", "CAT", "C*P", 0};
    size_t i;
    const char *text = "BEGIN_BEGIN_CAT_CIP_END_END";
    size_t text_length = strlen(text);
    size_t id;
    size_t state = 0;

    s = smack_create("test2", 1);
    smack_add_pattern(s, patterns[0], strlen(patterns[0]), 0,
                      SMACK_ANCHOR_BEGIN);
    smack_add_pattern(s, patterns[1], strlen(patterns[1]), 1, SMACK_ANCHOR_END);
    smack_add_pattern(s, patterns[3], strlen(patterns[2]), 2, 0);
    smack_add_pattern(s, patterns[3], strlen(patterns[3]), 3, SMACK_WILDCARDS);
    smack_compile(s);
    ...
    TEST(3, 19, "CIP");
    id = smack_search_next(s, &state, text, &i, text_length);
    // Fix for next step of test
    TEST(2, 19, "CAT");
    id = smack_search_next(s, &state, text, &i, text_length);
    TEST(SMACK_NOT_FOUND, 27, "$");
  ...
  {
    struct SMACK *s;
    const char *patterns[] = {"GET", "GETPUT", "GETPOST", "TPA", 0};
    size_t i;
    const char *text = "___GETPA__GETPUT__";
    size_t text_length = strlen(text);
    size_t id;
    size_t state = 0;

    s = smack_create("test3", 1);
    for (i = 0; patterns[i]; i++) {
      smack_add_pattern(s, patterns[i], strlen(patterns[i]), i, 0);
    }
    smack_compile(s);
...

  {
    struct SMACK *s;
    size_t i;
    const char *patterns[] = {"AT", "CAT", 0};
    const char *text = "_CAT_";
    size_t text_length = strlen(text);
    size_t id;
    size_t state = 0;

    s = smack_create("test5", 1);
    smack_add_pattern(s, patterns[0], strlen(patterns[0]), 0, 0);
    smack_add_pattern(s, patterns[1], strlen(patterns[1]), 1, 0);
    smack_compile(s);
...

  {
    struct SMACK *s;
    size_t i;
    const char *patterns[] = {"M**K", 0};
    const char *text = "M12K___M34K_";
    size_t text_length = strlen(text);
    size_t id;
    size_t state = 0;

    s = smack_create("test6", 1);
    smack_add_pattern(s, patterns[0], strlen(patterns[0]), 0,
                      SMACK_WILDCARDS | SMACK_ANCHOR_BEGIN);
    smack_compile(s);
...

  {
    struct SMACK *s;
    size_t i;
    const char *patterns[] = {"M*M", "BEGIN", "END", 0};
    const char *text = "BEGIN_BEGIN_MNM_END_END";
    size_t text_length = strlen(text);
    size_t id;
    size_t state = 0;

    s = smack_create("test7", 1);
    smack_add_pattern(s, patterns[0], strlen(patterns[0]), 0, SMACK_WILDCARDS);
    smack_add_pattern(s, patterns[1], strlen(patterns[1]), 1,
                      SMACK_ANCHOR_BEGIN);
    smack_add_pattern(s, patterns[2], strlen(patterns[2]), 2, SMACK_ANCHOR_END);
    smack_compile(s);
...

  {
    struct SMACK *s;
    size_t i;
    const char *patterns[] = {"BEGINEND"};
    const char *text = "BEGINEND";
    size_t text_length = strlen(text);
    size_t id;
    size_t state = 0;

    s = smack_create("test8", 1);
    smack_add_pattern(s, patterns[0], strlen(patterns[0]), 0,
