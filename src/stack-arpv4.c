...
struct ARP_IncomingRequest {
  ...
  ipv4address_t ip_src;
  ipv4address_t ip_dst;
...
static void proto_arp_parse(struct ARP_IncomingRequest *arp,
  ...
  /* parse the addresses */
  VERIFY_REMAINING(2 * (size_t)arp->hardware_length +
                   2 * (size_t)arp->protocol_length);
  arp->mac_src = px + offset;
  offset += arp->hardware_length;

  arp->ip_src = (ipv4address_t)px[offset + 0] << 24 |
                (ipv4address_t)px[offset + 1] << 16 |
                (ipv4address_t)px[offset + 2] << 8 |
                (ipv4address_t)px[offset + 3];
  offset += arp->protocol_length;

  arp->mac_dst = px + offset;
  offset += arp->hardware_length;

  arp->ip_dst = (ipv4address_t)px[offset + 0] << 24 |
                (ipv4address_t)px[offset + 1] << 16 |
                (ipv4address_t)px[offset + 2] << 8 |
                (ipv4address_t)px[offset + 3];
  // offset += arp->protocol_length;
...
int stack_arp_resolve(struct Adapter *adapter, const ipv4address_t *my_ipv4,
  ...
  for (;;) {
    ...
    if (time(0) > start + 1 && !is_arp_notice_given) {
      ipaddress_formatted_t fmt;
      ipv4address_fmt(&fmt, your_ipv4);
      LOG(LEVEL_WARNING, "[+] arping local router %s\n", fmt.string);
    ...
    if (adapter->is_vlan)
      proto_arp_parse(&response, px, 18, (size_t)length);
    else
      proto_arp_parse(&response, px, 14, (size_t)length);

    /* Is this an ARP packet? */
    if (!response.is_valid) {
      LOG(LEVEL_DEBUG, "[-] arp: etype=0x%04x, not ARP\n",
          px[12] * 256 + px[13]);
      continue;
    }
    ...
    if (response.opcode != 2) {
      LOG(LEVEL_DEBUG, "[-] arp: opcode=%u, not reply(2)\n", response.opcode);
      continue;
    }

    /* Is this response directed at us? */
    if (response.ip_dst != *my_ipv4) {
      LOG(LEVEL_DEBUG, "[-] arp: dst=%08x, not my ip 0x%08x\n", response.ip_dst,
          *my_ipv4);
      continue;
    }
    ...
    if (response.ip_src != *your_ipv4) {
      ipaddress_formatted_t fmt1, fmt2;
      ipv4address_fmt(&fmt1, &response.ip_src);
      ipv4address_fmt(&fmt2, your_ipv4);
      LOG(LEVEL_DEBUG, "[-] arp: target=%s, not desired %s\n", fmt1.string,
          fmt2.string);
      continue;
    }
    ...
    memcpy(your_mac_address->addr, response.mac_src, 6);
    {
      ipaddress_formatted_t fmt1, fmt2;
      ipv4address_fmt(&fmt1, &response.ip_src);
      macaddress_fmt(&fmt2, your_mac_address);
...
int stack_arp_incoming_request(struct stack_t *stack,
  ...
  if (request.opcode != 1) {
    LOG(LEVEL_DEBUG, "arp: opcode=%u, not request(1)\n", request.opcode);
    return -1;
  }
