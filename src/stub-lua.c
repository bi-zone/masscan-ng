...
int stublua_init(void) {
...

  {
    ...
    if (lib == NULL) {
      LOG(LEVEL_WARNING, "liblua: failed to load Lua shared library\n");
      LOG(LEVEL_WARNING, "    HINT: you must install Lua library\n");
    }
  }
...
#define DOLINK(name)                                                           \
  ...
  if (name == NULL)                                                            \
    LOG(LEVEL_WARNING, "liblua: %s: failed\n", #name);
#else
#define DOLINK(name)                                                           \
  name = (void (*)())dlsym(lib, #name);                                        \
  if (name == NULL)                                                            \
    LOG(LEVEL_WARNING, "liblua: %s: failed\n", #name);
#endif
