...
int pcap_init(void) {
  ...
  pl->is_available = false;
  pl->is_printing_debug = true;
...
#define DOLINK(PCAP_DATALINK, datalink)                                        \
  ...
  {
    static const char *possible_names[] = {"libpcap.so",
                                           "libpcap.A.dylib",
                                           "libpcap.dylib",
                                           "libpcap.so.0.9.5",
                                           "libpcap.so.0.9.4",
    ...
    for (i = 0; possible_names[i]; i++) {
      ...
      if (hLibpcap) {
        ...
        LOG(LEVEL_INFO, "[-] pcap: failed to load: %s\n", possible_names[i]);
      }
    }

    if (hLibpcap == NULL) {
      LOG(LEVEL_WARNING, "[-] FAIL: failed to load libpcap shared library\n");
      LOG(LEVEL_WARNING, "    [hint]: you must install libpcap or WinPcap\n");
    }
  }
...
#define DOLINK(PCAP_DATALINK, datalink)                                        \
  ...
  if (pl->datalink == NULL)                                                    \
    LOG(LEVEL_INFO, "pcap: pcap_%s: failed\n", #datalink);                     \
  if (pl->datalink == NULL)                                                    \
    pl->func_err = 1, pl->datalink = null_##PCAP_DATALINK;
#else
#define DOLINK(PCAP_DATALINK, datalink)                                        \
